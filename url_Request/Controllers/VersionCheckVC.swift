...
class VersionCheckVC: UIViewController {
    ...
    // MARK:- 'Methods'
    func styleField() {
        platformField.roundCorners(radius:5, bordorColor: UIColor(red: 102/255, green: 39/255, blue: 26/225, alpha: 1), borderWidth: 1)
        appIdField.roundCorners(radius:5, bordorColor: UIColor(red: 102/255, green: 39/255, blue: 26/225, alpha: 1), borderWidth: 1)
        versionNoField.roundCorners(radius:5, bordorColor: UIColor(red: 102/255, green: 39/255, blue: 26/225, alpha: 1), borderWidth: 1)
        buildNoField.roundCorners(radius:5, bordorColor: UIColor(red: 102/255, green: 39/255, blue: 26/225, alpha: 1), borderWidth: 1)
    }
    ...
    @IBAction func printBtn(_ sender: UIButton) {
        if platformField.text == "" || appIdField.text == "" || versionNoField.text == "" || buildNoField.text == "" {            let alert = Constants.showAlert(message: "Please Fill all fields")
            self.present(alert, animated: true, completion: nil)
        } else {
            let params: [String: AnyObject] = ["platform": platformField.text as AnyObject, "app_id": appIdField.text as AnyObject, "version_no": versionNoField.text as AnyObject, "environment": environment as AnyObject, "build_no": buildNoField.text as AnyObject ] // swiftlint:disable:this line_length
